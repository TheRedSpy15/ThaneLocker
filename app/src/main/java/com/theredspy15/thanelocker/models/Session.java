...
public class Session implements Serializable { // TODO: parcelable in the future
  ...
  public Session() {
    int randId =
        ThreadLocalRandom.current().nextInt(); // TODO: change to result of hash
                                               // after finished being created
    if (!savedSessionIds.contains(randId))
      setId(randId);
    else
      while (savedSessionIds.contains(randId))
        randId = ThreadLocalRandom.current().nextInt();
  ...
  public static void load() {
    ...
    if (json != null)
      savedSessions = gson.fromJson(
          json, new TypeToken<HashMap<Integer, Session>>() {}.getType());
    else
    ...
    if (json != null)
      savedSessionIds =
          gson.fromJson(json, new TypeToken<ArrayList<Integer>>() {}.getType());
    else
  ...
  public String
  getTotalDistance() { // returns string because I'm using it in only one spot
    double total = 0.0;
    ...
    String totalString =
        String.format("%.1f", total * 0.000621371192); // converts to miles
    if (total * 0.000621371192 < 10)
      return "0" + totalString;
    return totalString;
  }

  public String
  getTopSpeed() { // returns string because I'm using it in only one spot
    double top = 0.0;
    ...
    if (top < 10)
      return "0" + topString; // keep formatting
    return topString;
  }

  public String
  getAvgSpeed() { // returns string because I'm using it in only one spot
    double avg = 0.0;
  ...
  public ArrayList<SessionLocationPoint> getLocations() { return locations; }
  ...
  public String getCityName(Context context) {
    ...
    try {
      addresses = geocoder.getFromLocation(point.getLatitude(),
  ...
  public long getStart_millis() { return start_millis; }

  public void setStart_millis(long start_millis) {
    this.start_millis = start_millis;
  }

  public long getEnd_millis() { return end_millis; }

  public void setEnd_millis(long end_millis) { this.end_millis = end_millis; }

  public ArrayList<Integer> getBoard_ids() { return board_ids; }
  ...
  public ArrayList<Float> getElevationPoints() { return elevationPoints; }
