...
public class Image implements Serializable {
  ...
  /**
   * When doing any saving of an object that uses Image, it should save it to
   * the firebase storage, meant for files. NOT DATABASE! Save the image first,
   * then sent its data to nothing when saving the object containing the image.
   * Leaving only the location to reload it later
   */
    ...
    if (bytes != null && new App().isNetworkAvailable(context)) {
      ...
      uploadTask
          ...
          .addOnSuccessListener(
              taskSnapshot -> System.out.println("imageuploaded"));
    }
  }
  ...
  /**
   * Used to save image in memory preventing long loading, and offline issues.
   * But we can still properly store it in firestore
   */
