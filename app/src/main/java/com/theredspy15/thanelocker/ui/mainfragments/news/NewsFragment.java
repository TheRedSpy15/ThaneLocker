...
public class NewsFragment extends Fragment {
  ...
  public View onCreateView(@NonNull LayoutInflater inflater,
    ...
    Thread thread = new Thread(() -> {
      try {
        ...
        while (!isAdded()) {
        } // fixes a rare occurrence of crash when switching fragments too fast
  ...
  private void feedError() {
    ...
    TextView textView =
        new TextView(requireContext()); // no news feeds selected
    textView.setText(R.string.failed_news);
    textView.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);
    textView.setTextSize(18);
    requireActivity().runOnUiThread(
        () -> binding.feedLayout.addView(textView, layout));
    requireActivity().runOnUiThread(
    ...
    if (entries != null) {
      for (SyndEntry entry : entries) {
        ...
        button.setOnClickListener(
            v
            -> startActivity(
                new Intent(Intent.ACTION_VIEW, Uri.parse(entry.getUri()))));
  ...
  public List<SyndEntry> getFeed() throws FeedException {
    ...
    if (MainActivity.preferences.getBoolean("downhill254", true)) {
      ...
      try {
        ...
        e.printStackTrace();
        Snackbar
            .make(binding.getRoot(), R.string.failed_connect_downhill254,
    ...
    if (MainActivity.preferences.getBoolean("longboardbrand", false)) {
      ...
      try {
        ...
        e.printStackTrace();
        Snackbar
            .make(binding.getRoot(), R.string.failed_connect_longboardbrand,
                  Snackbar.LENGTH_LONG)
            .setAction("Action", null)
            .show();
      }
    ...
    if (MainActivity.preferences.getBoolean("basementskate", false)) {
      ...
      try {
        ...
        e.printStackTrace();
        Snackbar
            .make(binding.getRoot(), R.string.failed_connect_basementskate,
                  Snackbar.LENGTH_LONG)
            .setAction("Action", null)
            .show();
      }
