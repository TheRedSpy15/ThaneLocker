...
public class SessionsFragment extends Fragment {
  ...
  public void loadSessions() {
    ...
    if (Session.savedSessions != null &&
        Session.savedSessions.isEmpty()) { // no sessions
  ...
  private boolean checkPermission() { // TODO: check that gps is even on
    if (!PermissionChecker.checkPermissionLocation(requireContext()))
      PermissionChecker.requestPermissionLocation(requireActivity());
  ...
  public void loadStartSession(View view) {
    if (checkPermission()) {
      ...
      alertDialog.setButton(AlertDialog.BUTTON_NEGATIVE,
                            ...
                            (dialog, which) -> dialog.dismiss());
      alertDialog.setButton(
          AlertDialog.BUTTON_POSITIVE, getString(R.string.start),
          (dialog, which) -> {
            PowerManager powerManager =
                (PowerManager)requireContext().getSystemService(
                    Context.POWER_SERVICE);
            PowerManager.WakeLock wakeLock = powerManager.newWakeLock(
                PowerManager.PARTIAL_WAKE_LOCK, "ThaneLocker::WakelockTag");
  ...
  /**
   * sets record button text and color depending on whether or not a session is
   * being recorded actively *
   */
  void loadRecordingButtonIndicator() {
    if (isRecording) {
      binding.newSessionButton.setBackgroundColor(
          requireContext().getColor(R.color.recording_session));
  ...
  public void startService() {
    ...
    serviceIntent.putExtra(
        "description",
        getString(
            R.string.notification_description)); // maybe send list this way?
  ...
  public void stopService() {
    ...
    if (newSession.getLocations().size() > 0) {
      ...
      Profile.localProfile.addXp(
          (int)(Session.xpValue * newSession.getLocations().size()));
    } else
      Snackbar
          .make(binding.newSessionButton, R.string.not_enough_data,
