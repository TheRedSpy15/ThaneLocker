...
public class IntroActivity extends AppCompatActivity {
  ...
  private void signIntoFirestore() {
    ...
    if (currentUser == null) {
      MainActivity.mAuth.signInAnonymously().addOnCompleteListener(
          this, task -> {
  ...
  private TextView createDoneText(String text) {
    ...
    textView.setCompoundDrawablesRelativeWithIntrinsicBounds(
        null, null,
        ContextCompat.getDrawable(this, R.drawable.ic_baseline_check_24), null);
    return textView;
  }
  ...
  public void loadSessions() {
    ...
    // Create a query against the collection.
    Query query =
        sessionsRef.whereEqualTo("user_id", Profile.localProfile.getId());
    query.get().addOnCompleteListener(task -> {
      if (task.isSuccessful()) {
        for (QueryDocumentSnapshot document : task.getResult()) {
  ...
  public void loadBoard() {
    ...
    // Create a query against the collection.
    Query query =
        boardsRef.whereEqualTo("user_id", Profile.localProfile.getId());
    query.get().addOnCompleteListener(task -> {
      if (task.isSuccessful()) {
        for (QueryDocumentSnapshot document : task.getResult()) {
          Board board = document.toObject(Board.class);
          if (board.getImage() != null)
            board.getImage().downloadImage(this);
