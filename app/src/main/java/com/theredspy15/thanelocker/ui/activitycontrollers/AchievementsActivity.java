...
public class AchievementsActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    achievements = (ArrayList<Achievement>)getIntent().getSerializableExtra(
        "achievements");
    ...
    if (achievements != null) {
      ...
      for (Achievement achievement : achievements) {
        if (achievement != null) {
          ...
          case LEGENDARY:
            color = this.getColor(R.color.legendary);
            break;
          case RARE:
            color = this.getColor(R.color.rare);
            break;
          default:
            color = App.getThemeTextColor(this);
          }

          builder.append(achievement.getDescription()).append("\n");

          SpannableString str2 =
              new SpannableString("" + achievement.getRarity());
          str2.setSpan(new ForegroundColorSpan(color), 0, str2.length(), 0);
          builder.append(str2);

          builder.append(" (XP:")
              .append(String.valueOf(achievement.getXp()))
              .append(")");
          ...
          this.runOnUiThread(() -> {
            ...
            // ObjectAnimator animation = ObjectAnimator.ofFloat(text, View.X,
            // View.Y); animation.setDuration(2000); animation.start();
          });
        }
      }
      Collections.reverse(achievements);
      ...
      if (achievements.isEmpty()) { // no achievements
        ...
        this.runOnUiThread(
            () -> binding.achievementLayout.addView(textView, layout));
      }
    }
  }
}